...
public class MediaCodecDecoder implements SurfaceTexture.OnFrameAvailableListener {
    ...
    // if (m_verbose)
    // Log.d(TAG, "Submitted frame to decoder input buffer " + inputBufIndex +
    // ", size=" + chunkSize);
    //
    // m_inputBufferQueued = true;
    // ++m_pendingInputFrameCount;
    ...
    // dequeueTimeoutUs = TIMEOUT_USEC;
    // } else {
    // // NOTE: Too few input frames has been queued and the decoder has not yet
    // seen input EOS
    // // wait dequeue for too long in this case is simply wasting time.
    // dequeueTimeoutUs = 0;
    // }
    ...
    // Log.d(TAG, "Decoder output format changed: " + newFormat);
    // } else if (decoderStatus < 0) {
    // Log.e(TAG, "Unexpected result from decoder.dequeueOutputBuffer: " +
    // decoderStatus);
    // return ERROR_FAIL;
    // } else { // decoderStatus >= 0
    // if (m_verbose) {
